...
> 本分支基于 [xpdigital/ios-location-spoofer](https://github.com/xpdigital/ios-location-spoofer)，增加 NAS 选点后即时写入 Loon、离线保留最后一次有效配置，以及手动启停 Docker 示例。
...
此分支为 NAS `location-picker` 增加了 Loon 本机即时同步。插件导入地址：

```text
https://raw.githubusercontent.com/riddikuluswen/ios-location-spoofer/refs/heads/main/ios-location-spoofer.lnplugin
```
...
```text
Location spoofer instant sync -> enabled=true, lat=纬度, lng=经度
```

如果页面提示“Loon 即时同步未触发”，检查插件是否启用、页面是否由装有该插件的 Loon 接管流量，以及配置服务器是否仍为 `http://192.168.31.10:3007`。
